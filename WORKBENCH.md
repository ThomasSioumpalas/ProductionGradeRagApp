...
- `llm.py` calls Groq using strict JSON-schema output, bounded retries and a configurable supported model. The default is `openai/gpt-oss-120b`. If that model fails Groq's server-side schema generation, it falls back once to JSON mode; Pydantic still validates the identical local schema before any facts are accepted. Incomplete or refused responses are rejected.
