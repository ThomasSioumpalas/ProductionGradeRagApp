...
3. Wait for extraction. The service splits every PDF page into overlapping, page-aware chunks. It ranks financial chunks, selects a bounded and diverse set for structured extraction, then sends metric groups from those chunks to the model. All pages stay available for source viewing and questions. The counter shows planned provider requests; dense outputs may be split into smaller requests if the model reaches its output limit. Scanned/image-only documents need OCR first.
...
- `documents.py` reads sorted page text with PyMuPDF, then makes overlapping chunks that retain their original PDF page. It ranks financial terms and numeric density and chooses a configurable maximum of 24 diverse chunks. It does not run expensive geometric table detection on every page. The extraction engine sends one 2,500-character chunk per request and splits relevant workbook metrics into groups of ten (at most four requests for a 36-metric statement chunk). If a response hits the token limit, only that request is retried with smaller metric groups. Output field lengths and fact count are schema-bounded to 30 facts per request.
- `llm.py` calls Groq using strict JSON-schema output, low reasoning effort, up to eight bounded retries and a configurable supported model. The default is `openai/gpt-oss-120b`. If that model fails Groq's server-side schema generation, it retries once without provider formatting; Pydantic still validates the identical local schema before any facts are accepted. Incomplete or refused responses are rejected with the provider finish reason and token usage retained in the job error. Rate-limit retries honor Groq's stated retry window, including daily-token resets, with a small timing margin.
